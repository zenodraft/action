...
const zenodraft = __importStar(require("zenodraft"));
const read_inputs = () => {
...
const main = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        let version_id;
        if (concept_id === '') {
            version_id = yield zenodraft.deposition_create_concept(token, sandbox, verbose);
        }
        else {
            version_id = yield zenodraft.deposition_create_version(token, sandbox, concept_id, verbose);
        }
        core.endGroup();
        if (upsert_doi === true) {
            core.startGroup('upserting doi');
            const prereserved_doi = yield zenodraft.deposition_show_prereserved(token, sandbox, version_id, verbose);
        ...
        if (filenames === '') {
            ...
            for (const filename of filenames.split(' ')) {
                yield zenodraft.file_add(token, sandbox, version_id, filename, verbose);
            }
        }
        core.endGroup();
        ...
        if (metadata !== '') {
            ...
            yield zenodraft.metadata_update(token, sandbox, version_id, metadata, verbose);
            core.endGroup();
        }
        if (publish === true) {
            core.startGroup('publishing');
            yield zenodraft.deposition_publish(token, sandbox, version_id, verbose);
            core.endGroup();
        }
