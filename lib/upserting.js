...
const push_changes_from_upsert = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    yield exec_1.exec('git', ['commit', '-m', 'zenodraft updated the file CITATION.cff with the prereserved doi']);
    yield exec_1.exec('git', ['push']);
});
const show_github_payload = () => {
    ...
    if (github.context.eventName === 'release') {
        ...
        if (release_event_payload.action === 'published') {
            release_published_eventhandler(release_event_payload);
            return;
        }
        else {
...
const release_published_eventhandler = (payload) => {
...
const workflow_dispatch_eventhandler = (payload) => {
    core.info(JSON.stringify(payload, null, 4));
