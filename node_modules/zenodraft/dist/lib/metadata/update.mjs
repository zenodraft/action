...
export const metadata_update = (token, sandbox, version_id, filename, verbose = false) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    yield deposition_show_details(token, sandbox, version_id, verbose);
    ...
    const response = yield fetch(`${api}${endpoint}`, init);
    if (response.ok !== true) {
        throw new Error(`(errid 13) Something went wrong on ${method} to ${api}${endpoint}: ${response.status} - ${response.statusText}`);
    }
