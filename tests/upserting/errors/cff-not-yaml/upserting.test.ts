...
beforeEach(() => {
    ...
    fs.copyFileSync(src('CITATION.bin'), dest('CITATION.cff'))
...
test('upserting a doi',() => {
    ...
    try {
        ...
        expect(err.message).toBe('Could not parse the contents of CITATION.cff as YAML. Try https://yamllint.com to fix the problem.')
    }
})
